...
class StagiaireController extends Controller
{
    ...
    public function deleteStagiaire($id){
        ...
        if($Stagiaire->delete()){
            return redirect('/liste')->with('msg','hetha stagiaire w rt7na mino');
        }else{
            return 'error d\'ajout';
        }
    }

    public function afficheform($id = null){
        $stagiaire = null;
        if ($id) {
            $stagiaire = Stagiaire::find($id);
            if (!$stagiaire) 
        ...
        return view('form', compact('stagiaire')); //compact -> liste avec les donnees de stagiaire de la bdd
    }
    ...
    return redirect()->route('stagiaire.liste');
    }
